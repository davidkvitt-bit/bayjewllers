        ...
        <div className="flex h-16 md:h-20 items-center justify-between">
            ...
            <Sheet>
              ...
              <SheetContent side="left" className="w-full sm:w-80 bg-background">
                <div className="flex flex-col h-full pt-8">
                  <Link href="/" className="text-2xl tracking-[0.2em] font-light mb-12">
                    BAY JEWELLERS
                  </Link>
          ...
          <div className="hidden md:flex md:items-center md:gap-8 lg:gap-12">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-sm tracking-wider font-light hover:text-accent transition-colors uppercase"
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <Button variant="ghost" size="icon" className="hover:bg-transparent relative">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-accent text-[10px] flex items-center justify-center">
                  {cartCount}
                </span>
              )}
